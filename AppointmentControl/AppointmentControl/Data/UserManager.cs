...
namespace AppointmentControl.Data
{
    public class UserManager
    {
        ...
        private readonly MobileServiceClient _azureClient;

        public UserManager()
        {
            _azureClient = new MobileServiceClient(
                Constants.AzureApplicationUrl,
                Constants.AzureApplicationKey);

            _table = _azureClient.GetTable<User>();
        }
        ...
        public async Task<ObservableCollection<User>> GetTodoItemsAsync()
        {
            try
            {
        ...
        public async Task SaveTaskAsync(User user)
        {
            if (user.Id == null)
            {
                ...
                //TodoViewModel.TodoItems.Add(user);
            }
            else
            {
